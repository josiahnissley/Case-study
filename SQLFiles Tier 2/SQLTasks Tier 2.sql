...
SELECT sub.member_name, sub.court
FROM (
    SELECT CONCAT(m.firstname, ' ', m.surname) AS member_name, f.name AS court
    FROM Bookings AS b
    INNER JOIN Facilities AS f ON b.facid = f.facid
    INNER JOIN Members AS m ON b.memid = m.memid
    WHERE f.name LIKE 'Tennis Court%'
) AS sub
GROUP BY sub.member_name, sub.court
...
AND (((b.memid =0) AND (b.guestcost * b.slots >30))
OR ((b.memid !=0) AND (f.membercost * b.slots >30)))
INNER JOIN Members ON b.memid = m.memid
ORDER BY cost DESC
